...
export async function createApp() {
  ...
  const testerHtml = process.env.VERCEL
    ? join(process.cwd(), "tester", "index.html")
    : join(import.meta.dir, "..", "tester", "index.html");

  app
    .get("/tester", async () => {
      try {
        await access(testerHtml, constants.F_OK);
        const html = await readFile(testerHtml, "utf-8");
